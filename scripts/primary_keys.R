...
## variables that form primary keys for a given dataset. For most, this is SEQN. This is a list of exceptions.
...
pkey <- function(x)
{
    switch(x,
           ...
           DR1IFF_F = , DR1IFF_G = , DR1IFF_H = , DR1IFF_I = , DR1IFF_J = c("SEQN", "DR1ILINE"),
           P_DR2IFF = , DR2IFF_C = , DR2IFF_D = , DR2IFF_E = ,
           DR2IFF_F = , DR2IFF_G = , DR2IFF_H = , DR2IFF_I = , DR2IFF_J = c("SEQN", "DR2ILINE"),
           ...
           DSQIDS_E = , DSQIDS_F = , DSQIDS_G = , DSQIDS_H = , DSQIDS_I = c("SEQN", "DSDSUPID"),
           ## NCHS supplement id variable name changed in cycle J?
           P_DS1IDS = , P_DS2IDS = , P_DSQIDS = , DS1IDS_J = , DS2IDS_J = , DSQIDS_J = c("SEQN", "DSDPID"),
           ## Miscellaneous
           ...
           ## have neither SEQN nor SAMPLEID
...
for (x in tables)
{
    ...
    if (anyDuplicated(nhanes(x, translated = FALSE)[pk])) cat("not unique: (", paste(pk, collapse = ", "), ")\n")
}
